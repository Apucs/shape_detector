...
def get_jaw_parameters(frame, face_coordinates, thickness_factor):
    ...
    # Jaw width
    cv2.line(frame,
             (face_coordinates[58][0], face_coordinates[58][1]),
             (face_coordinates[288][0], face_coordinates[288][1]),
             color=(255, 255, 0),
    ...
    print(theta1, end_x1, end_y1)
    print(theta2, end_x2, end_y2)

    intersect = line_intersect(face_coordinates[58][0], face_coordinates[58][1],
                               end_x1, end_y1,
                               face_coordinates[288][0], face_coordinates[288][1],
                               end_x2, end_y2
                               )

    cv2.circle(frame, (int(intersect[0]), int(intersect[1])), (thickness_factor + 1), color=(255, 255, 255),
               thickness=-1)
...
def calculate_face_shape_metrics(frame, face_coordinates, thickness_factor):
    ...
    cv2.line(frame
             , (face_coordinates[54][0], face_coordinates[54][1])
             , (face_coordinates[284][0], face_coordinates[284][1])
             , color=(255, 255, 0)
             , thickness=thickness_factor
             )
    ...
    cv2.line(frame
             , (face_coordinates[234][0], face_coordinates[234][1])
             , (face_coordinates[454][0], face_coordinates[454][1])
             , color=(255, 255, 0)
             , thickness=thickness_factor
             )
    ...
    cv2.line(frame
             , (face_coordinates[152][0], face_coordinates[152][1])
             , (x_p, y_p)
             , color=(255, 255, 0)
             , thickness=thickness_factor
             )
    ...
    # Chin to mouth
    chin_to_mouth_height = calculate_Euclidean_Distance(face_coordinates, 17, 152)
    cv2.line(frame
             , (face_coordinates[17][0], face_coordinates[17][1])
             , (face_coordinates[152][0], face_coordinates[152][1])
             , color=(255, 0, 0)
             , thickness=thickness_factor
             )

    jaw_width, jaw_angle = get_jaw_parameters(frame, face_coordinates, thickness_factor)
...
def get_face_shape(forehead_width, face_width, face_height, chin_to_mouth_height, jaw_width, jaw_angle):
    ...
    for i in range(1):
        ...
        # Pear / Heart / Oval / Diamond / Triangle
            ...
            print(f'Face Width to Jaw Width Ratio = {face_w_to_jaw_w_ratio:.2f}')

            # Face width to Forehead Width Ratio
            face_w_to_forehead_w_ratio = face_width / forehead_width
            print(f'Face Width to Forehead Width Ratio = {face_w_to_forehead_w_ratio:.2f}')

            # Jaw width to Forehead Width Ratio
            jaw_w_to_forehead_w_ratio = jaw_width / forehead_width
            print(f'Jaw Width to Forehead Width Ratio = {jaw_w_to_forehead_w_ratio:.2f}')

            chin_h_to_jaw_w_ratio = chin_to_mouth_height / jaw_width
            print(f'Chin Height to Jaw Width Ratio = {chin_h_to_jaw_w_ratio:.2f}')
    ...
    params = {
        ...
        , "Jaw Width": jaw_width
        , "Chin To Mouth Height": chin_to_mouth_height
        , "Jaw Angle": jaw_angle
...
def layout_shape_parameters(params, shape):
    """
    ...
    # font = ImageFont.true-type(os.path.join(config.FONTS_PATH,'OpenSansEmoji.ttf')
    #                          ,20,encoding='uni')

    font = ImageFont.truetype("OpenSansEmoji.ttf", 20, encoding='uni')
    # for idx, (i) in enumerate(params):
    #     label = "{}={:.2f}".format(i, params.get(i))
    #     draw.text((20, 40 * idx), label, font=font, embedded_color=True)

    label = '{}: "{}"'.format("Detected shape", shape)
    draw.text((20, 40 * (len(params)//2)), label, font=font, embedded_color=True)
...
def calculate_thickness_factor(frame):
    thickness_factor = (frame.shape[0] * frame.shape[1]) / ((frame.shape[0] + frame.shape[1]) * 100)
...
def detect_face_shape(input_path: str, display_output: bool = False):
    ...
    print("Detected faces", faces.detections)
    output = []
    output_info = []
    ...
    print("detected landmarks:", len(landmarks))
    ...
    for idx, (face_detected, face_landmarks) in enumerate(zip(faces.detections, landmarks)):
        ...
        forehead_width, face_width, face_height, chin_to_mouth_height \
            , jaw_width, jaw_angle = calculate_face_shape_metrics(frame, face_coordinates, thickness_factor)

        params, shape = get_face_shape(forehead_width, face_width, face_height, chin_to_mouth_height, jaw_width,
                                       jaw_angle)
        if shape:
            shape_parameters_layout = layout_shape_parameters(params, shape)
            ...
            if shape_parameters_layout is not None:
                combined_img = cv2.hconcat([cv2.resize(roi_face_color, (400, 400), cv2.INTER_CUBIC)
                                               , cv2.resize(shape_parameters_layout, (400, 400), cv2.INTER_CUBIC)])
                ...
                output_filepath = config.OUTPUT_PATH+"result_" + str(input_path.split("/")[-1].split(".")[0]) + ".png"
...
if __name__ == '__main__':
    ...
    args = parse_args()
    detect_face_shape(input_path=args['input_path'], display_output=args['display_output'])
